...
placing_rapids = False
selected_turret = None
selected_rapid = None

#load images
#map
map_image = pg.image.load('levels/rtd.png').convert_alpha()
#turret spritesheets
turret_spritesheets = []
rapid_spritesheets = pg.image.load(f'assets/images/turrets/rapid_1.png').convert_alpha()
for x in range(1, c.TURRET_LEVELS + 1):
  turret_sheet = pg.image.load(f'assets/images/turrets/turret_{x}.png').convert_alpha()
  turret_spritesheets.append(turret_sheet)
#individual turret image for mouse cursor
cursor_turret = pg.image.load('assets/images/turrets/cursor_turret.png').convert_alpha()
cursor_rapid = pg.image.load('assets/images/turrets/cursor_rapid.png').convert_alpha()
#enemies
enemy_images = {
...
rapid_image = pg.image.load('assets/images/buttons/buy_turret.png').convert_alpha()
#gui
...
with open('levels/Road_Death.tmj') as file:
...
def create_turret(mouse_pos):
  ...
  if world.tile_map[mouse_tile_num] == 48 or world.tile_map[mouse_tile_num] == 121 or world.tile_map[mouse_tile_num] == 98 or world.tile_map[mouse_tile_num] == 97:
...
def create_rapid(mouse_pos):
  ...
  if world.tile_map[mouse_tile_num] == 48 or world.tile_map[mouse_tile_num] == 121 or world.tile_map[mouse_tile_num] == 98 or world.tile_map[mouse_tile_num] == 97:
    #check that there isn't already a turret there
    space_is_free = True
    for rapid in rapid_group:
      if (mouse_tile_x, mouse_tile_y) == (rapid.tile_x, rapid.tile_y):
        space_is_free = False
    #if it is a free space then create turret
    if space_is_free == True:
      new_rapid = Rapid(rapid_spritesheets, mouse_tile_x, mouse_tile_y, shot_fx)
      rapid_group.add(new_rapid)
      #deduct cost of turret
      world.money -= c.RAPID_BUY_COST
...
rapid_group = pg.sprite.Group()

#create buttons
turret_button = Button(c.SCREEN_WIDTH + 30, 120, buy_turret_image, True)
cancel_button = Button(c.SCREEN_WIDTH + 50, 240, cancel_image, True)
upgrade_button = Button(c.SCREEN_WIDTH + 5, 235, upgrade_turret_image, True)
...
rapid_button = Button(c.SCREEN_WIDTH + 30, 175, buy_turret_image, True)
...
while run:
  ...
  if game_over == False:
    ...
    rapid_group.update(enemy_group, world)
  ...
  if game_over == False:
    ...
    if selected_turret:
     ...
     #RAPIDDD
    draw_text(str(c.RAPID_BUY_COST), text_font, "grey100", c.SCREEN_WIDTH + 215, 165)
    screen.blit(coin_image, (c.SCREEN_WIDTH + 260, 160))
    if rapid_button.draw(screen):
      placing_rapids = True
    #if placing turrets then show the cancel button as well
    if placing_rapids == True:
      #show cursor turret
      cursor_rect = cursor_turret.get_rect()
      cursor_pos = pg.mouse.get_pos()
      cursor_rect.center = cursor_pos
      if cursor_pos[0] <= c.SCREEN_WIDTH:
        screen.blit(cursor_rapid, cursor_rect)
      if cancel_button.draw(screen):
        placing_rapids = False
    #if a turret is selected then show the upgrade button
    if selected_rapid:
      #if a turret can be upgraded then show the upgrade button
      if selected_rapid.upgrade_level < c.RAPID_LEVELS:
        #show cost of upgrade and draw the button
        draw_text(str(c.UPGRADE_COST), text_font, "grey100", c.SCREEN_WIDTH + 215, 195)
        screen.blit(coin_image, (c.SCREEN_WIDTH + 260, 190))
        if upgrade_button.draw(screen):
    ...
    if event.type == pg.MOUSEBUTTONDOWN and event.button == 1:
      ...
      if mouse_pos[0] < c.SCREEN_WIDTH and mouse_pos[1] < c.SCREEN_HEIGHT:
        ...
        if placing_rapids == True:
          #check if there is enough money for a turret
          if world.money >= c.BUY_COST:
            create_rapid(mouse_pos)
        else:
          selected_rapid = select_rapid(mouse_pos)
